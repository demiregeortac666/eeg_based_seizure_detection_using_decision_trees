...
# run_pipeline.sh
#
# Bu script EEG analiz iş akışındaki tüm adımları sırayla çalıştırır:
# 1. Özellik çıkarma
...
# 1. Özellik çıkarma (tek bir dosya, hasta veya tüm veri seti)
process_features() {
    echo -e "\n${BLUE}=== 1. EEG'den özellik çıkarma =====${NC}"
    
    if [ -n "$FILE_ARG" ]; then
        echo -e "${YELLOW}Belirtilen EDF dosyası işleniyor: $FILE_ARG${NC}"
        python scripts/extract_all_features.py --file "$FILE_ARG"
        
        if [ $? -ne 0 ]; then
            echo -e "${RED}❌ Özellik çıkarma adımında hata! İşlem durduruluyor.${NC}"
            exit 1
        fi
        
        # Tek bir dosya işlendiğinde tam otomatik pipeline'a devam edemeyiz
        PARTIAL_MODE=true
        echo -e "${YELLOW}⚠️ Tek dosya işlendiğinden tam pipeline çalıştırılamıyor.${NC}"
        echo -e "${YELLOW}   Oluşturulan 'output/features/feat_*.csv' dosyasını kullanarak:"
        echo -e "   python scripts/create_balanced_dataset.py --input [DOSYA_YOLU]${NC}"
        
    elif [ -n "$PATIENT_ARG" ]; then
        echo -e "${YELLOW}Belirtilen hasta(lar) işleniyor: $PATIENT_ARG${NC}"
        python scripts/extract_all_features.py --patient $PATIENT_ARG
        ...
        echo -e "${YELLOW}Tüm veri seti işleniyor...${NC}"
        echo -e "${YELLOW}⚠️ Bu işlem uzun sürebilir (saatler).${NC}"
        echo -e "${YELLOW}   İptal etmek için Ctrl+C tuşlarını kullanabilirsiniz.${NC}"
        sleep 3  # Kullanıcıya iptal şansı vermek için kısa bir bekleme
    ...
    if [ "$PARTIAL_MODE" = false ]; then
        echo -e "${GREEN}✓ Özellik çıkarma tamamlandı${NC}"
    fi
}
...
# 4.5 NaN Değerlerini Temizleme
clean_data() {
    if [ "$PARTIAL_MODE" = true ]; then
        return
    fi
    
    echo -e "\n${BLUE}=== 4.5. NaN değerlerini temizleme =====${NC}"
...
train_models() {
    ...
    echo -e "\n${BLUE}=== 5. Model eğitimi ve değerlendirme =====${NC}"
...
# 6. JMP formatına dönüştürme
prepare_for_jmp() {
    if [ "$PARTIAL_MODE" = true ]; then
        return
    fi
    
    echo -e "\n${BLUE}=== 6. JMP için veri hazırlama =====${NC}"
...
show_help() {
    echo "Kullanım: ./run_pipeline.sh [SEÇENEKler]"
    echo ""
    echo "Seçenekler:"
    echo "  --file EDF_DOSYASI      Belirli bir EDF dosyasını işle"
    echo "  --patient ID1 [ID2...]  Belirli hasta ID'lerini işle (örn: chb01 chb02)"
    echo "  --skip-models           Model eğitim adımını atla"
    echo "  --help, -h              Bu yardım mesajını göster"
    echo ""
    echo "Örnekler:"
    echo "  ./run_pipeline.sh                         # Tüm veri setini işle"
    echo "  ./run_pipeline.sh --patient chb01         # Sadece chb01 hastasını işle"
    echo "  ./run_pipeline.sh --file path/to/file.edf # Tek bir EDF dosyasını işle"
    exit 0
}
